...
class Trainer(object):
    ...
    def __init__(self, args):
        ...
        rela_matrix = torch.tensor(np.load(os.path.join(self.data_path, 'embeddings', self.modelname + '_' + self.semantic_of_rel))['relaM'].astype('float32'))

        if self.args.cat_exp:
            rela_matrix_exp = torch.tensor(np.load(os.path.join(self.data_path, 'embeddings', 'rela_matrix_exp.npz'))['relaM'].astype('float32'))
            rela_matrix = torch.cat((rela_matrix, rela_matrix_exp), dim=1)

        print('##LOADING RELATION MATRIX##')
        # self.rela_matrix = rela_matrix.astype('float32')
        self.rela_matrix = rela_matrix
    ...
    def load_pretrain(self):
        self.Extractor.load_state_dict(torch.load(self.save_path + '/' + self.embed_model + '_' + self.splitname + '_Extractor', map_location='cuda:'+ str(self.args.device)))
    ...
    def load(self):
        self.Generator.load_state_dict(torch.load(self.save_path + '/' + self.embed_model + '_' + self.splitname + '_Generator', map_location='cuda:'+ str(self.args.device)))
        self.Discriminator.load_state_dict(torch.load(self.save_path + '/' + self.embed_model + '_' + self.splitname + '_Discriminator', map_location='cuda:'+ str(self.args.device)))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1, (self.train_times+1))):
            ...
            if epoch >= 100 and epoch % self.eval_every == 0:
    ...
    def eval(self, mode='dev', meta=True, epoch=0):
        ...
        test_candidates = json.load(open(self.data_path + '/' + self.splitname + '_' + self.args.test_candidates))
        ...
        for query_ in tqdm(sorted(test_candidates.keys())):
            ...
            description = torch.unsqueeze(description, 0)
            # description = np.expand_dims(description, axis=0)
            descriptions = description.repeat(self.test_sample, 1)
            ...
            relation_vecs_norm = relation_vecs / relation_vecs.norm(dim=-1, keepdim=True)
            ...
            for e1_rel, tail_candidates in test_candidates[query_].items():
                ...
                if meta:
                    ...
                    candidate_vecs_norm = candidate_vecs / candidate_vecs.norm(dim=-1, keepdim=True)
                    ...
                    # print(relation_vecs.shape)
                    scores = torch.mm(candidate_vecs_norm, relation_vecs_norm.t())

                    scores = scores.mean(dim=1)

                    assert scores.shape == (len(query_pairs),)
                
                # sort = list(torch.argsort(scores.cpu()).numpy())[::-1]
                sort = torch.argsort(scores, descending=True)
                sort = list(sort.cpu().numpy())
                rank = sort.index(0) + 1
                if rank <= 10:
                    hits10.append(1.0)
                    hits10_.append(1.0)
